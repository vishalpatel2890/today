...
<critical>Continue until all tasks/subtasks are checked, then MANDATORY HALT at Test Gate (Step 5.5) for human frontend validation. Do NOT mark story complete without explicit PASS from user.</critical>
...
Expected ready-for-dev or in-progress. Continuing anyway...
  ...
  <step n="5" goal="Mark task complete, track review resolutions, and update story">
    ...
    <action if="no tasks remain"><goto step="5.5">Test Gate</goto></action>
  </step>

  <step n="5.5" goal="MANDATORY Test Gate - Human Frontend Validation">
    <critical>This step is MANDATORY. Do NOT skip. Do NOT proceed to Step 6 without explicit PASS.</critical>

    <action>Verify all Tasks/Subtasks are marked [x]</action>
    <action>Extract Frontend Test Gate section from story file</action>

    <output>
═══════════════════════════════════════════════════════════════════════════════
🧪 FRONTEND TEST GATE: {{story_key}}
═══════════════════════════════════════════════════════════════════════════════
...
**Prerequisites:**
{{prerequisites_from_test_gate}}

**Test Steps:**
{{test_steps_table_from_story}}
...
───────────────────────────────────────────────────────────────────────────────
❓ **FEEDBACK NEEDED**:
{{feedback_questions_from_story}}

───────────────────────────────────────────────────────────────────────────────
⏸️  **PAUSED** - Please test in browser and reply:

   **PASS** - All success criteria met, feature works as expected
   **FAIL** - [describe what didn't work]
   **PARTIAL** - Core feature works, minor issues: [list them]

═══════════════════════════════════════════════════════════════════════════════
    </output>

    <action>HALT - Wait for user response</action>

    <check if="user response contains PASS">
      <action>Add to Dev Agent Record → Completion Notes: "✅ Test Gate PASSED by {{user_name}} ({{date}})"</action>
      <goto step="6">Completion</goto>
    </check>

    <check if="user response contains FAIL">
      <action>Parse failure description</action>
      <action>Add to Dev Agent Record → Debug Log: "❌ Test Gate FAILED: {{user_failure_description}}"</action>
      <action>Create new subtask(s) in Tasks/Subtasks for each failure</action>
      <action>Uncheck relevant completed tasks if implementation was incorrect</action>
      <goto step="2">Re-implement</goto>
    </check>

    <check if="user response contains PARTIAL">
      <action>Add to Dev Agent Record → Completion Notes: "⚠️ Test Gate PARTIAL: {{user_minor_issues}}"</action>
      <action>Create follow-up tasks for minor issues (mark as deferred)</action>
      <goto step="6">Completion</goto>
    </check>

    <check if="user response unclear">
      <ask>Please clarify: Did the frontend testing PASS, FAIL, or PARTIAL? Include details.</ask>
    </check>
  </step>
