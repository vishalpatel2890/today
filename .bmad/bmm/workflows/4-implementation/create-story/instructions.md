...
Will update existing story file rather than creating new one.
  ...
  <step n="6.5" goal="Generate Frontend Test Gate">
    <critical>Every story MUST have testable frontend behavior. If story has no UI, create API test gate instead.</critical>

    <action>From acceptance criteria, derive user-actionable test steps:
      - Convert each AC into manual browser test steps
      - Use specific UI element names (button labels, form fields, page titles)
      - Include expected visual feedback for each action
    </action>

    <action>Generate test prerequisites:
      - What must be running (frontend URL, backend)
      - Required user role/authentication state
      - Any seed data needed
    </action>

    <action>Create success criteria checklist:
      - Map each AC to observable UI state
      - Include "no console errors" and "no network failures"
    </action>

    <action>Write 3-5 feedback questions:
      - Primary action completable?
      - Response time acceptable?
      - Error messages clear?
      - Any UX friction?
    </action>

    <template-output file="{default_output_file}">frontend_test_gate</template-output>
  </step>
