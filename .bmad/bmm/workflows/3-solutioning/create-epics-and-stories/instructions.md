...
- **FRONTEND-TESTABLE** (see critical requirement below)

</action>

<critical>⚠️ FRONTEND-TESTABLE REQUIREMENT: Every story MUST result in something a human can test in the browser.

**VALIDATION CHECK for each story:**
- Can a human open the app and verify this story works?
- Is there a UI element, page, or interaction to test?
- If backend-only, does it enable a frontend feature in the SAME story?

**ANTI-PATTERN (REJECT):**
- "Create database schema for users" (no frontend test)
- "Implement auth API endpoints" (no frontend test)
- "Add validation service" (no frontend test)

**CORRECT PATTERN:**
- "User can register with email" (form → submit → success message)
- "User can log in and see dashboard" (form → submit → dashboard visible)
- "User can create new item" (button → form → item appears in list)

If a story cannot be tested in the browser, RESTRUCTURE IT to include the frontend component that uses the backend work.
</critical>
